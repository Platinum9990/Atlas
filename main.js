...
function homePage() {
  ...
  <style>
    ...
    .ch-bg {
      ...
      background: url('https://images.unsplash.com/photo-1497435334941-8c899ee9e8e9?w=1600&q=80') center/cover no-repeat;
      opacity: 0.18;
      z-index: 0;
    }
    .ch-bg-gradient {
      position: absolute; inset: 0;
      background: linear-gradient(to right, rgba(8,15,30,0.85) 40%, rgba(8,15,30,0.3) 100%);
      z-index: 0;
    }
    .ch-scanline {
    ...
    @keyframes chscan { 0%{top:0;opacity:0} 5%{opacity:0.25} 95%{opacity:0.25} 100%{top:100%;opacity:0} }
    ...
    .ch-eyebrow::before { content: ''; width: 1.5rem; height: 1px; background: var(--blue); }
    .ch-headline {
    ...
    .ch-actions {
      ...
      align-items: center;
      flex-wrap: wrap;
      animation: fadeSlideIn 0.7s ease 0.7s both;
    }
    ...
    .ch-hud-val span { color: var(--blue); font-size: 1.2rem; }
    .ch-hud-sub {
      font-family: 'IBM Plex Mono', monospace;
      font-size: 0.5rem;
      color: var(--slate-dim);
    ...
    .ops-band::-webkit-scrollbar { display: none; }
    .ops-cell {
      flex: 1;
      min-width: 180px;
      padding: 2rem 2rem;
    ...
    .ops-num span { color: var(--blue); font-size: 1.5rem; }
    .ops-unit { font-family: 'IBM Plex Mono', monospace; font-size: 0.55rem; letter-spacing: 0.15em; text-transform: uppercase; color: var(--blue); }
    .ops-desc { font-size: 0.72rem; color: var(--slate-dim); line-height: 1.5; margin-top: 0.25rem; }

    /* ── EDITORIAL SERVICES LIST ── */
    .svc-list { border-top: 1px solid rgba(14,165,233,0.12); }
    .svc-list-header {
      display: flex;
    ...
    .svc-row:hover { background: rgba(14,165,233,0.04); }
    .svc-row:hover .svc-row-arrow { transform: translateX(6px); }
    .svc-row:hover .svc-row-img { opacity: 1; max-width: 220px; }
    .svc-row-num {
      padding: 2rem 1.5rem;
      font-family: 'IBM Plex Mono', monospace;
      font-size: 0.62rem;
    ...
    .svc-row-body { padding: 2rem 2.5rem; }
    .svc-row-title {
      font-family: 'Barlow Condensed', sans-serif;
    ...
    .svc-row-tags { display: flex; gap: 0.5rem; flex-wrap: wrap; }
    .svc-row-tag {
      font-family: 'IBM Plex Mono', monospace;
      font-size: 0.5rem;
    ...
    .svc-row-tag + .svc-row-tag::before { content: '·'; margin-right: 0.5rem; }
    .svc-row-right {
      display: flex;
      align-items: center;
      gap: 2rem;
    ...
    /* ── MAGAZINE PROJECTS ── */
    .mag-projects { background: var(--navy-2); }
    .mag-header {
      display: flex;
      justify-content: space-between;
      align-items: flex-end;
    ...
    .mag-main-overlay {
      position: absolute; inset: 0;
      background: linear-gradient(to top, rgba(8,15,30,0.92) 0%, rgba(8,15,30,0.2) 60%);
    }
    .mag-main-body {
      position: absolute;
    ...
    .mag-side-card:hover { background: var(--navy-3); }
    .mag-side-card::after {
      content: '';
      position: absolute;
    ...
    .mag-side-card:hover::after { width: 2px; }
    .mag-proj-service { font-family: 'IBM Plex Mono', monospace; font-size: 0.55rem; letter-spacing: 0.12em; text-transform: uppercase; color: var(--blue); margin-bottom: 0.5rem; }
    .mag-proj-title {
      font-family: 'Barlow Condensed', sans-serif;
      font-size: 1.3rem;
      font-weight: 700;
    ...
    .mag-proj-loc { font-size: 0.72rem; color: var(--slate-dim); margin-bottom: 0.75rem; }
    .mag-proj-cap {
      font-family: 'Barlow Condensed', sans-serif;
      font-size: 1.5rem;
      font-weight: 700;
    ...
    .manifesto-item-text { font-size: 0.78rem; line-height: 1.8; color: var(--slate-dim); }
  </style>

  <!-- ── COMMAND HORIZON HERO ── -->
  <div class="ch-hero">
    <div class="ch-bg"></div>
    <div class="ch-bg-gradient"></div>
    <div class="ch-scanline"></div>
    <div class="ch-vertical-label">ATLAS INFRASTRUCTURE · EPC · O&M · BOOT · ENERGY · ADVISORY</div>

    <!-- LEFT — Stacked headline -->
    <div class="ch-left">
      <div class="ch-eyebrow">Est. 2005 · Lagos, Nigeria · 23 Countries</div>
      <div class="ch-headline">
        <span class="hl-1">Power</span>
        <span class="hl-2">Africa.</span>
        <span class="hl-3">Built</span>
        <span class="hl-4">to last.</span>
      </div>
      <p class="ch-sub">Africa's leading infrastructure contractor — delivering EPC, O&M, BOOT and energy solutions across Sub-Saharan Africa. $4.2B portfolio. Zero missed milestones.</p>
    ...
    <!-- RIGHT — HUD data panel -->
    <div class="ch-right">
      <div class="ch-hud-panel">
        <div class="ch-hud-cell">
          <div class="ch-hud-label">MW Delivered</div>
          <div class="ch-hud-val">1,200<span>+</span></div>
          <div class="ch-hud-sub">Across 23 countries</div>
        </div>
        <div class="ch-hud-cell">
          <div class="ch-hud-label">Portfolio Value</div>
          <div class="ch-hud-val">$4.2<span>B</span></div>
          <div class="ch-hud-sub">EPC · O&M · BOOT</div>
        </div>
        <div class="ch-hud-cell">
          <div class="ch-hud-label">Asset Uptime</div>
          <div class="ch-hud-val">98.7<span>%</span></div>
          <div class="ch-hud-sub">5-yr O&M average</div>
        </div>
        <div class="ch-hud-cell">
          <div class="ch-hud-label">LTIs — 5 Years</div>
          <div class="ch-hud-val">0</div>
          <div class="ch-hud-sub">Zero. Not negotiable.</div>
        </div>
      </div>
    </div>

    <div class="ch-coords">6.5244° N 3.3792° E // Lagos HQ<br>// SYSTEM ACTIVE · 10 LIVE PROJECTS</div>
  </div>
  ...
  <!-- ── OPERATIONS BAND ── -->
  <div class="ops-band">
    <div class="ops-cell" data-index="01">
      <div class="ops-unit">Projects Delivered</div>
      <div class="ops-num">47</div>
      <div class="ops-desc">Across Sub-Saharan Africa, North Africa & the Middle East</div>
    </div>
    <div class="ops-cell" data-index="02">
      <div class="ops-unit">Years Operating</div>
      <div class="ops-num">20<span>+</span></div>
      <div class="ops-desc">Founded Lagos 2005. Never missed a completion milestone</div>
    </div>
    <div class="ops-cell" data-index="03">
      <div class="ops-unit">Asset Uptime</div>
      <div class="ops-num">98.7<span style="font-size:1.2rem">%</span></div>
      <div class="ops-desc">5-year O&M average. Industry benchmark is 94%</div>
    </div>
    <div class="ops-cell" data-index="04">
      <div class="ops-unit">LTIs — 5 Years</div>
      <div class="ops-num">0</div>
      <div class="ops-desc">Zero Lost Time Injuries across all global sites since 2020</div>
    </div>
    <div class="ops-cell" data-index="05">
      <div class="ops-unit">Countries Active</div>
      <div class="ops-num">23</div>
      <div class="ops-desc">Permanent in-country teams in 9 of those markets</div>
    </div>
  </div>
  ...
  <div class="svc-list">
    <div class="svc-list-header">
      <div>
        <div class="tag" style="margin-bottom:0.75rem">Capabilities</div>
        <h2 class="section-title">What we<br><em>deliver.</em></h2>
      </div>
      <a href="#/services" class="btn-outline-blue" data-link>Full Service Detail</a>
    </div>
    ${services.map((s,i) => `
    <div class="svc-row" onclick="location.hash='/services'">
      <div class="svc-row-num">${String(i+1).padStart(2,'0')}</div>
      <div class="svc-row-body">
        <div class="svc-row-title">${s.title}</div>
        <div class="svc-row-tags">${s.tags.slice(0,4).map(t=>`<span class="svc-row-tag">${t}</span>`).join('')}</div>
      </div>
      <div class="svc-row-right">
        <img src="${s.img}" class="svc-row-img" alt="${s.abbr}">
        <div class="svc-row-arrow">→</div>
      </div>
    </div>`).join('')}
  </div>

  <!-- ── MAGAZINE PROJECTS ── -->
  <div class="mag-projects">
    <div class="mag-header">
      <div>
        <div class="tag" style="margin-bottom:0.75rem">Selected Commissions</div>
        <h2 class="section-title">Recent<br><em>projects.</em></h2>
      </div>
      <a href="#/projects" class="btn-outline-blue" data-link>Full Project Map</a>
    </div>
    <div class="mag-grid">
      <div class="mag-main" onclick="location.hash='/projects'">
        <img src="https://images.unsplash.com/photo-1581094794329-c8112a89af12?w=1200&q=80" class="mag-main-img" alt="${projects[0].title}">
        <div class="mag-main-overlay"></div>
        <div class="mag-main-body">
          <div class="mag-proj-service">${projects[0].service} · ${projects[0].year}</div>
          <div class="mag-proj-title" style="font-size:clamp(1.8rem,3vw,2.8rem)">${projects[0].title}</div>
          <div class="mag-proj-loc">📍 ${projects[0].loc}</div>
          <div style="display:flex;align-items:center;gap:1.5rem;margin-top:0.5rem">
            <div class="mag-proj-cap">${projects[0].capacity}</div>
            <div class="mag-proj-cap" style="color:var(--amber);font-size:1rem">${projects[0].value}</div>
          </div>
        </div>
      </div>
      ${projects.slice(1,3).map((p,i) => `
      <div class="mag-side-card" onclick="location.hash='/projects'">
        <div class="mag-proj-status" style="${p.status==='active'?'color:var(--blue);border-color:var(--blue)':'color:var(--amber);border-color:var(--amber)'}">${p.status}</div>
        <div class="mag-proj-service">${p.service}</div>
        <div class="mag-proj-title">${p.title}</div>
        <div class="mag-proj-loc">📍 ${p.loc}</div>
        <div class="mag-proj-cap">${p.capacity} <span style="font-size:1rem;color:var(--slate-dim)">${p.value}</span></div>
      </div>`).join('')}
    </div>
  </div>
  ...
  <!-- ── MANIFESTO ── -->
  <div class="manifesto">
    <div class="manifesto-bg-text">ATLAS</div>
    <div class="manifesto-inner">
      <div>
        <div class="tag" style="margin-bottom:2rem">Why ATLAS</div>
        <div class="manifesto-quote">
          Twenty years.<br>
          <em>Zero excuses.</em><br>
          One continent.
        </div>
        <div style="margin-top:3rem;display:flex;gap:1rem">
          <a href="#/why-us" class="btn-primary" data-link>Our Differentiators</a>
          <a href="#/about" class="btn-outline-blue" data-link>Our Story</a>
        </div>
      </div>
      <div class="manifesto-right">
        ${[
          ['01','In-Country Permanence','We don\'t fly in for projects and fly out. Permanent teams in 9 countries with local procurement, regulatory relationships and community trust built over decades.'],
          ['02','Single-Point Accountability','EPC, O&M, BOOT and advisory under one roof. One contract. One team. No interface disputes between contractors.'],
          ['03','Financial Engineering','Investment-grade balance sheet enabling us to structure and finance BOOT projects without depending on third-party capital.'],
          ['04','Proven Safety Culture','Zero LTIs in five years. Not a compliance metric — a genuine operating standard that every person on every site owns personally.']
        ].map(([n,t,d]) => `
        <div class="manifesto-item">
          <div class="manifesto-item-num">${n}</div>
          <div>
            <div class="manifesto-item-title">${t}</div>
            <div class="manifesto-item-text">${d}</div>
          </div>
        </div>`).join('')}
      </div>
    </div>
  ...
  <div class="cta-strip">
    <div>
      ...
      <p>Our business development team operates across five offices. Start the conversation today.</p>
    </div>
    <a href="#/contact" class="btn-navy" data-link>Get in Touch</a>
  </div>`;
}
